...
{
    $q1         = mysqli_query($con,"SELECT `updated`,`showroom_view`,`lunas`,`pemilik`,`alamat`,`tgl_sold`,`no`,`foto`,NOW() as `wkt` FROM `$db`.`hewan` where `id_hwn`='$hwn'");
...
if (isset($_POST['keep'])) {
	...
	$fo = $_POST['foto'];
	catat("keep",$nmus,$db);
    mysqli_query($con,"UPDATE `$db`.`hewan` SET `pemilik` = '$nma', `alamat` = '$alm', `lunas` = 'keep', `tgl_sold` = now() WHERE `hewan`.`id_hwn` = '$idny'");
...
if (isset($_POST['lunas'])) {
	...
	$fo = $_POST['foto'];
	catat("lunas",$nmus,$db);
    mysqli_query($con,"UPDATE `$db`.`hewan` SET `pemilik` = '$nma', `alamat` = '$alm', `lunas` = 'lunas', `tgl_sold` = now() WHERE `hewan`.`id_hwn` = '$idny'");
    $info = '<div class="col-lg-4">
